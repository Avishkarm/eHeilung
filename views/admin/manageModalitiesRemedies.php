...
if(isset($_GET["modalitiesId"]) && !in_array($_GET["modalitiesId"], $blanks)){
	$modalitiesId = $_GET["modalitiesId"];	
} else {
...
if(isset($_POST["submit"]) && $_POST["submit"]==1){
	...
	foreach ($_POST as $modalitiesId => $modalitiesRemedies) {
		if($modalitiesRemedies!=1){
			$query = "UPDATE modalities SET modalitiesRemedies='".$modalitiesRemedies."' WHERE modalitiesId=".$modalitiesId." ";
...
if($type=="Modalities"){
	$modalitiesRemedies = getModalitiesRemedies($modalitiesId, $conn);

	if(noError($modalitiesRemedies)){
		$modalitiesRemedies = $modalitiesRemedies["errMsg"];
	} else {
		printArr("Error fetching modalities remedies");
		exit;	
	}
	//get reaction choices
	$modalities = getAllModalities($conn);
	if(noError($modalities)){
		$modalities = $modalities["errMsg"];
	} else {
		printArr("Error fetching causation reactions data");
		exit;
	}
} 
$remediesData = (array)json_decode($modalitiesRemedies["modalitiesRemedies"], true);
                ...
                <section class="content" style="text-align: center;">
                    <div>
						<form action="" method="post">
							...
							<?php
							foreach($modalities as $modalitiesId=>$modalitiesDetails){
								
							?>								
								<p>
									<label><?php echo $modalitiesDetails["modalitiesName"]; ?> modalities Remedies: </label>

									<?php
									$temp= $modalitiesDetails["modalitiesName"];
									...
									//printArr($temp);printArr($remediesData);
									 ...
									 ?>
										<input type="text" value="<?php echo $modalitiesDetails["modalitiesRemedies"]; ?>" name="<?php echo $modalitiesDetails["modalitiesId"]; ?>" id="<?php echo $modalitiesDetails["modalitiesName"]; ?>" /> <button type="button" onclick="manageRemedies('<?php echo $modalitiesDetails["modalitiesName"]; ?>')">Manage Remedies</button>
