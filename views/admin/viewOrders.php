...
$query = sprintf("Select * from order_details");

$result = runQuery($query, $conn);
$res = [];
if(noError($result)){
    while($row=mysql_fetch_assoc($result["dbResource"])){
        $res[$row['order_id']] = $row;
    }
    $returnArr['errCode'][-1] = -1;
    $returnArr['errMsg'] = $res;    
    $orderDetails = $returnArr['errMsg'];
}else{
    $returnArr['errCode'][1] = 1;
    $returnArr['errMsg'] = "Error Fetching Order Details";
    ...
    <body class="skin-blue">
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side"> 
                <!-- Main content -->
                <section class="content" style="text-align: center;overflow: scroll;">
                    ...
                    <table class="rowTable">
                        ...
                        <tr>
                            ...
                            <td><?php 
                                    $region_id = getManageRegionById($conn, $value['region_id']);
                                    if(noError($region_id)){
                                        echo join(',', array_map(function (array $ele) { return $ele['region_name']; }, $region_id['errMsg']));
                                    }else{
                                        printArr($region_id['errMsg']);
                                    }
                                ?>
                            </td>
                            <td><?php
                                    $currId = getCurrencyByID($conn, $value['currency_id'],"longforms,shortforms");

                                    if(noError($currId)){
                                        $currId = $currId['errMsg'][0];
                                        echo $currId['longforms']." (".$currId['shortforms'].")";
                                    }else{
                                        printArr($currId['errMsg']);
                                    }
                                ?>
                            </td>
