...
$query="SELECT region_id,region_name From manageRegion Where status=1"; 
$result=runQuery($query,$conn);
$res=array();
if(noError($result)){    
    while($row=mysqli_fetch_assoc($result['dbResource'])){
        $res[$row['region_id']]=$row['region_name'];
    }
}
//printArr($res);
$options = json_encode( $res);//die;
//printArr($options);
?>
<!DOCTYPE html>
<html>
    ...
    <body class="skin-blue">
        ...
        <script>
            $(window).load(function () {
                console.log({ 'Patient_Only': 'Patient Only', 'Doctor_Only': 'Doctor Only' , 'Both': 'Both' }   );
                $('#RubricsTableContainer').jtable({
                    ...
                    actions: {
                        listAction: '<?php echo '../../controllers'; ?>/admin/pricecrud/list.php?planId=<?php echo $_GET['pid'];?>',
                        createAction: '<?php echo '../../controllers'; ?>/admin/pricecrud/create.php?planId=<?php echo $_GET['pid'];?>',
                        updateAction: '<?php echo '../../controllers'; ?>/admin/pricecrud/update.php?planId=<?php echo $_GET['pid'];?>',
                        deleteAction: '<?php echo '../../controllers'; ?>/admin/pricecrud/delete.php?planId=<?php echo $_GET['pid'];?>'
                    },
                    fields: {
                        priceId: {
                            key: true,
