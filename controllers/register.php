...
if(noError($conn)){
	$conn = $conn["errMsg"];
	$userInfo=array();
	$userInfo['user_first_name']=strip_tags($_POST["user_first_name"]);
	$userInfo['user_last_name']=strip_tags($_POST["user_last_name"]);
	$userInfo['user_email']=strip_tags($_POST["user_email"]);
	$userInfo['cpassword']=strip_tags($_POST["cpassword"]);
	$userInfo['user_mob']=strip_tags($_POST["user_mob"]);
	$userInfo['user_type_id']=3;
	...
	if (filter_var($userInfo['user_email'], FILTER_VALIDATE_EMAIL)) {
		...
		if(isset($_POST["user_first_name"]) && isset($_POST["user_last_name"]) && isset($_POST["user_email"]) && isset($_POST["user_password"]) && isset($_POST["cpassword"])){
			...
			if($userInfo['cpassword']!=$password){
				...
				$user=getUserInfoWithType($_POST["user_email"],3,$conn);
				...
				if(noError($user)){
					...
					///Exist user activation link(target, link)
				...
				if($user['pass_code']!=="*****"){
					...
					$msg = "verifiedlogin.php?passkey=$com_code&username=$email";
                  ...
                  <br>
					...
					//echo 'hi'
					echo '<a href="resendActivation.php?url='.$url.'&email='.$email.'&from='.$from.'&subject='.$subject.'&msg='.$msg.'"><button>Resend Verification</button></a>';
					$resend="notActive";
					                              
                   $msg = "User Already Exit......Your Account is not Active Please Activate it.";						
                  ...
                  <br>
						...
						//$return_arr=sendNotifications(1, $userInfo["user_email"], $from, $subject, $message);
					
						if(noError($return_arr)){
							$returnArr['errCode']=-1;
           					$msg = "Registered Successfully <br> Check Email for Activation link";
							$returnArr['errMsg']=$msg;	
							$redirectURL='../'.$newUI_URL['loginSignUP'];
						}else{
							$returnArr['errCode']=$return_arr['errCode'];
           					$msg = "Unsuccessful Email Sent";
							$returnArr['errMsg']=$return_arr['errMsg'];	
							$redirectURL='../'.$newUI_URL['loginSignUP'];
						}
					} 
					else {
