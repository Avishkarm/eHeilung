...
include("../models/followUpModel.php");
...
if(isset($_POST) && !empty($_POST)){
	if($_POST['type']=="endFollowup"){
		$case_id=$_POST['case_id'];
		$followup_status="closed";
		$closeFollowup=updateFollowupstatus($case_id,$followup_status,$conn);

		echo json_encode($closeFollowup['errMsg']);
	}else if($_POST['type']=="retakeCase"){
		$case_id=$_POST['case_id'];
		$getCaseData=getCaseData($case_id,$conn);
		//printArr($getCaseData); 
		$kes_case_id=$getCaseData['errMsg'];
		//die;
		$getCaseSheetData=getCaseSheetData($case_id,$conn);
		//printArr($getCaseSheetData);
		foreach ($getCaseSheetData['errMsg'] as $key => $value) {
			$createBlankSheet=createBlankSheet($kes_case_id,$value,$conn);
			//printArr($createBlankSheet);
			if(noError($createBlankSheet)){
				$returnArr['errCode']=-1;
				$returnArr['errMsg']="Blank sheet created successfully";
			}else{
				$returnArr['errCode']=2;
				$returnArr['errMsg']=$createBlankSheet['errMsg'];
			}
		}
		//die;
		$returnArr['case_id']=$kes_case_id;
