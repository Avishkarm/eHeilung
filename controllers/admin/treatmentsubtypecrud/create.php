...
if(noError($conn)){
    ...
    if(isset($_SESSION["admin"]) && !in_array($_SESSION["admin"], $blanks)){
        ...
        $treatmentTypeId = $_GET['type_id'];
        $newTreatment = addNewTreatmentSubtype($newTreatmentName, $treatmentTypeId,$conn);
        if(noError($newTreatment)){
            $returnArr["Result"]="OK";
            $returnArr["Record"]=array("treatmentId"=>$newTreatment["errMsg"],
